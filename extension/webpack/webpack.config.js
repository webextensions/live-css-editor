...
module.exports = function (env) {
    ...
    const configs = [
        {
            entry: { 'alert': './alert.js' }
        },
        {
            entry: { 'background-magicss': './background-magicss.js' }
        },
        {
            entry: { 'external-editor-base': './external-editor-base.js' }
        },
        {
            entry: { 'load-editor': './scripts/load-editor.js' }
            // // TODO: FIXME:
            // // Somehow, the `load-editor` bundle is not working as expected (the code editor is not loading). There
            // // is chance that some of the library codes which affect the global scope (like `globalThis`) might not
            // // be loading in the correct order. Hence, we are not using the `vendors` bundles (for this and other
            // // "entry" scripts) for now.
            // useVendorsBundle: true,
            // vendorsBundleName: 'load-editor.vendors',
            // vendorsBundleTest: /[\\/]node_modules[\\/]|[\\/]extension[\\/]scripts[\\/]3rdparty[\\/]/
        },
        {
            entry: { 'load-reapply': './scripts/load-reapply.js' }
        },
        {
            entry: { 'main': './src/main.js' }
            // useVendorsBundle: true,
            // vendorsBundleName: 'main.vendors'
        },
        {
        ...
        const webpackConfig = {
            ...
            optimization: {
                ...
                splitChunks: {
                    cacheGroups: {
                        vendors: (function () {
                            if (config.useVendorsBundle) {
                                return {
                                    name: config.vendorsBundleName, // The name of the vendors bundle
                                    test: config.vendorsBundleTest || /[\\/]node_modules[\\/]/, // Specify the path to the vendors' code
                                    chunks: 'all'
                                };
                            } else {
                                return {};
